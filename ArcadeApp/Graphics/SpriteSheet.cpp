...
{
	bool canLoadImage = mBMPImage
		.Load(App::Singleton().GetBasePath() + std::string("Assets\\") + name + ".bmp");
	bool canLoadSpriteSections =
		LoadSpriteSections(App::Singleton().GetBasePath() + std::string("Assets\\") + name + ".txt");

	return canLoadImage && canLoadSpriteSections;
}
