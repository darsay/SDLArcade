...
AnimatedSprite::AnimatedSprite():mnoptrSpriteSheet(nullptr), mPosition(Vec2D::Zero)
{
}
...
void AnimatedSprite::Draw(Screen& theScreen)
{
	...
	theScreen.Draw(*mnoptrSpriteSheet, frame.frame, frame.offset, frameColor);
...
{
	AARectangle bbox = { mPosition, static_cast<unsigned>(Size().GetX()), static_cast<unsigned>(Size().GetY()) };
	return bbox;
}