...
bool FileCommandLoader::LoadFile(const std::string& filePath) const
{
	std::ifstream inFile;
		...
		if ((commandPos = line.find(":")) != std::string::npos)
		{
			...
			for (auto& command : mCommands)
			{
				if (commandStr == command.command) {
					if (command.commandType == COMMAND_ONE_LINE) {
...
Color FileCommandLoader::ReadColor(const ParseFuncParams& params)
{
	...
	const int r = std::stoi(params.line.substr(params.dilimitPos, (nextSpacePos - params.dilimitPos)));

	size_t lastSpacePos = nextSpacePos;
	nextSpacePos = params.line.find_first_of(" ", lastSpacePos + 1);
	const int g = std::stoi(params.line.substr(lastSpacePos + 1, (nextSpacePos - lastSpacePos)));

	lastSpacePos = nextSpacePos;
	nextSpacePos = params.line.find_first_of(" ", lastSpacePos + 1);
	const int b = std::stoi(params.line.substr(lastSpacePos + 1, (nextSpacePos - lastSpacePos)));
	const int a = std::stoi(params.line.substr(nextSpacePos + 1));
