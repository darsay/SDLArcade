...
void BreakOut::Draw(Screen& screen)
{
	...
	Line2D cutoff = { Vec2D(0, mYCutoff), Vec2D{static_cast<float>(App::Singleton().Width()), mYCutoff} };
...
{
	auto path = App::GetBasePath() + "Assets\\BreakoutLevels.txt";
	mLevels = BreakOutGameLevel::LoadLevelsFromFile(path);

	mYCutoff = App::Singleton().Height() - 2 * Paddle::PADDLE_HEIGHT;

	AARectangle paddleRect = {
		Vec2D(App::Singleton().Width() / 2 - Paddle::PADDLE_WIDTH / 2 , App::Singleton().Height() - 3 * Paddle::PADDLE_HEIGHT),
...
bool BreakOut::HasBallPassedCutoffY() const
{
