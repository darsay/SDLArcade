...
ButtonOptionsScene::ButtonOptionsScene(const std::vector<std::string>& optionsNames,
	const std::vector<Button::ButtonAction>& buttonActions, const Color& textColor):
	mHighlightedOption(0)
{
	const BitmapFont& font = App::Singleton().GetFont();

	for(const auto& name : optionsNames)
	{
...
void ButtonOptionsScene::Init()
{
	...
	confirmAction.key = GameController::ActionKey();
	confirmAction.action = [this](uint32_t dt, InputState state)
		{
			if (GameController::IsPressed(state))
			{
	...
	mGameController.AddInputActionForKey(confirmAction);
...
{
	int previousHighlighted = mHighlightedOption;

	mHighlightedOption = (mHighlightedOption + 1) % mButtons.size();

	HighLightCurrentButton(previousHighlighted);
}
...
void ButtonOptionsScene::HighLightCurrentButton(int previousHighlighted)
{
	mButtons[previousHighlighted].SetHighlighted(false);
	mButtons[mHighlightedOption].SetHighlighted(true);
}
